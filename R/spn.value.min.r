...
spn.value.min <-
  function(spn,
    ...
    return(spn.value.min.aux(spn$root, evi, eps, eps.gauss, use_memory))
  }
## auxiliary function that does the job
spn.value.min.aux <-
  function(node, evi, eps, eps.gauss, use_memory) {
    ...
    if (use_memory && exists(evi.id, envir = node$memory)) {
      ...
      if (node$type == 3) {
        ...
        for (nod in 1:l) {
          res_count <-
            spn.value.min.aux(node$children[[nod]], evi, eps, eps.gauss, use_memory)
      ...
      if (node$type == 4) {
        ...
        i.set <- order(vals.min)
        for (i in i.set) {
          if (total > eps) {
            if(round(total, 7) > round(eps, 7))
              print(paste0("min-->", total, "--->", eps))
            vals[i] <- vals[i] + eps
            total <- total - eps
          } else {
            vals[i] <- vals[i] + total
        ...
        res <- logsumexp(log(vals) + vals.min)
        if (use_memory) {
          env_poke(node$memory, evi.id, res) # Add result to hashtable
        }
        return(list("res" = res, "count" = count))
