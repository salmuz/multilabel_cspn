...
## INPUT: spn (an SPN), evi (a config of variables for which
## we want to compute the marginal probability for that instantiation),
...
spn.value.min <-
  function(spn,
    ...
    if (sum(spn$ncat[evi$scope] * spn$ncat[evi$scope] < spn$ncat[evi$scope] *
            evi$value) > 0 ||
        sum((spn$ncat[evi$scope] > 1) &
            (evi$value < 1)) > 0 ||
        sum(spn$ncat[evi$scope] * (round(evi$value) != evi$value)) > 0) {
    ...
    return(spn.value.min.aux(spn$root, evi, eps, eps.gauss, use_memory, idm_version))
  }
...
spn.value.min.aux <-
  function(node,
    ...
    if (node$type == 1) {
      #'leaf-indicator'
      ## for leaf nodes, return log(1) unless
      ## the var of this leaf appears in the evi config and is not compatiable with it
    ...
    if (use_memory && exists(evi.id, envir = node$memory)) {
      ...
      if (node$type == 3) {
        ...
        for (nod in 1:l) {
          res_count <-
            spn.value.min.aux(node$children[[nod]],
                              evi,
                              eps,
                              eps.gauss,
                              use_memory,
          ...
          res <-
            res + res_count$res ## log product is sum of logs
          if (res == -Inf)
            break
        }
        if (use_memory) {
      ...
      if (node$type == 4) {
        #'sum'
        ## for sum nodes, combine the results from
        ## the children with the appropriate weights
        l <- length(node$children)
        count <- 1
        vals.min <- rep_len(0, l)
        for (nod in 1:l) {
          res_count <-
            spn.value.min.aux(node$children[[nod]],
                              evi,
                              ...
                              idm_version)
          count <- count + res_count$count
          vals.min[nod] <- res_count$res
        }
        
        # different solutions e-contamination and imprecise dirichlet
        #     min_w x⋅w  st.  w \in C (credal set of interval values)
        if (!idm_version) {
          weigth.opt <- e.contamination(node$weight, eps, x = vals.min)
        } else{
          weigth.opt <-
            idm.contamination(node$weight, node$total_weight, eps, x = vals.min)
        }
        # minimum value in logarithm log(w*⋅x)
        res <- logsumexp(log(weigth.opt) + vals.min)
        if (use_memory) {
          env_poke(node$memory, evi.id, res) # Add result to hashtable
        }
        return(list("res" = res, "count" = count))
